...
class JobController extends Controller
{
    ...
    public function pendingApprovalJobs()
    {
        $companies = DataArrayHelper::companiesArray();
        $countries = DataArrayHelper::defaultCountriesArray();
        return view('admin.job_pending.index')
                        ->with('companies', $companies)
                        ->with('countries', $countries);
    }
    ...
    {
        $jobs = Job::select([
                    'jobs.id', 'jobs.company_id', 'jobs.title', 'jobs.description', 'jobs.country_id', 'jobs.state_id', 'jobs.city_id', 'jobs.career_level_id', 'jobs.salary_from', 'jobs.salary_to', 'jobs.hide_salary', 'jobs.functional_area_id', 'jobs.job_type_id', 'jobs.num_of_positions', 'jobs.gender_id', 'jobs.expiry_date', 'jobs.degree_level_id', 'jobs.job_experience_id', 'jobs.is_active', 'jobs.is_featured',
        ]);
    ...
    public function fetchPendingJobsData(Request $request)
    {
        $jobs = Job::select([
                    'jobs.id', 'jobs.company_id', 'jobs.title', 'jobs.description', 'jobs.country_id', 'jobs.state_id', 'jobs.city_id', 'jobs.career_level_id', 'jobs.salary_from', 'jobs.salary_to', 'jobs.hide_salary', 'jobs.functional_area_id', 'jobs.job_type_id', 'jobs.num_of_positions', 'jobs.gender_id', 'jobs.expiry_date', 'jobs.degree_level_id', 'jobs.job_experience_id', 'jobs.is_active', 'jobs.is_featured',
        ]);
        return Datatables::of($jobs)
                        ...
                        ->addColumn('action', function ($jobs) {
                            /*                             * ************************* */
                            $approveTxt = 'Approve';
                            $approveHref = 'makeApprove(' . $jobs->id . ');';
                            $approveIcon = 'check';
                            return '
				<div class="btn-group">
					<button class="btn blue dropdown-toggle" data-toggle="dropdown" aria-expanded="false">Action
					...
					<ul class="dropdown-menu">
						...
						<li>
						<a href="javascript:void(0);" onClick="' . $approveHref . '" id="onclickActive' . $jobs->id . '"><i class="fa fa-' . $approveIcon . '" aria-hidden="true"></i>' . $approveTxt . '</a>
						</li>																																	
					</ul>
				</div>';
                        })
    ...
    public function makeApproveJob(Request $request)
    {
        $id = $request->input('id');
        try {
